...
pub static INGEST_WAL_WRITE_TIME: Lazy<HistogramVec> = Lazy::new(|| {
    HistogramVec::new(
        HistogramOpts::new("ingest_wal_write_time", "ingest wal write time")
            .namespace(NAMESPACE)
            .buckets(vec![
                0.2, 0.5, 1.0, 5.0, 10.0, 20.0, 50.0, 100.0, 200.0, 500.0, 1000.0, 2000.0,
            ])
...
fn register_metrics(registry: &Registry) {
    ...
    registry
        ...
        .expect("Metric registered");
    registry
        .register(Box::new(INGEST_WAL_WRITE_TIME.clone()))
        .expect("Metric registered");
